	...
	internal static class HttpParser
	{
		public static HttpRequest Parse(byte[] buffer)
		{
			...
			if (request.Method == "POST" || request.Method == "PUT" || request.Method == "PATCH")
			{
				...
				else if (headers.GetValue("Content-Type")?.StartsWith("application/x-www-form-urlencoded") ?? false)
				{
					string urlEncoded = Encoding.UTF8.GetString(buffer.Skip(bodyStartIndex).ToArray());
					string[] urlEncodedSplited = urlEncoded.Split('&');
					NameValueCollection parameters = new NameValueCollection();
					foreach (var parameter in urlEncodedSplited)
					{
						string[] splited = parameter.Split('=');
						parameters.Add(splited[0], splited[1]);
					}
					request.Items["R_PARAMETERS"] = parameters;
				}
